...
export function ModelGuide({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <section className="guide-panel" aria-labelledby="guide-title">
      ...
      <label className={`check-row${!notesChecked ? " gi-pulse" : ""}`}><input type="checkbox" checked={notesChecked} onChange={(event) => setNotesChecked(event.target.checked)} /> 이 안내를 읽었어요. 화살표는 열이 전체적으로 가는 방향이에요.</label>
