...
import { areAcceptedFrameDirections, areAcceptedFrameModes, areExactlyRequiredEvidenceSelected, frameComparisonStatus, frameDirectionAnswers, type FrameDirectionAnswer } from "../domain/learningRecord";
...
function actionButtonClass(isReady = false) {
  return `button primary${isReady ? " gi-pulse" : ""}`;
}
...
export function ScenarioFlow({ scenario, number, onComplete }: { scenario: ThermalScenario; number: number; onComplete: () => void }) {
  ...
  const selectedRequiredEvidenceCount = scenario.requiredEvidenceIds.filter((id) => evidence.includes(id)).length;
  const allEvidenceSelected = areExactlyRequiredEvidenceSelected(scenario, evidence);
  const hasDistractorEvidence = evidence.some((id) => !scenario.requiredEvidenceIds.includes(id));
  ...
  return <section className="scenario-flow" aria-labelledby="scenario-title">
    <div className="scenario-controls">
      <nav className="progress-tracker" aria-label={`사건 ${number}의 학습 단계`}>
        ...
        })}</ol><p className="progress-hint">옆으로 밀어 다음 단계도 볼 수 있어요.</p>
      </nav>
    ...
    {stage === "condition" && <section className="condition-sheet"><h2>시작 조건표를 확인해요</h2><ul>{scenario.controlledConditions.map((condition) => <li key={condition}>{condition}</li>)}</ul>{scenario.changedCondition && <p className="changed-condition">바뀌는 조건: {scenario.changedCondition}</p>}<button type="button" className={actionButtonClass(true)} onClick={() => changeStage("prediction")}>시작 온도 확인했어요</button></section>}
    {stage === "prediction" && <section className="decision-panel"><h2>처음에는 어느 쪽으로 갈까요?</h2><p>시작 온도와 조건을 보고 열이 가는 방향을 골라요.</p>{predictionAnswers.map((answer, index) => <fieldset className="choice-group" key={answer.id}><legend>{predictionAnswers.length > 1 ? `정거장 ${"ABC"[index]}에서 열이 가는 방향` : "열이 가는 방향 예측"}</legend>{directionOptions(answer).map((option) => <label key={option.id}><input type="radio" name={`prediction-${scenario.id}-${answer.id}`} checked={prediction[answer.id] === option.id} onChange={() => setChoice(setPrediction, prediction, answer.id, option.id)} /> {directionLabel(scenario, option.fromId, option.toId, option.id)}</label>)}</fieldset>)}<button type="button" className={actionButtonClass(choicesComplete(predictionAnswers, prediction))} disabled={!choicesComplete(predictionAnswers, prediction)} onClick={recordPrediction}>예측 기록하기</button></section>}
    {stage === "timeline" && <><div className="observation-panel"><p className="section-kicker">이번에 볼 것</p><h2>{observationPrompt(scenario, revealedFrameIndex)}</h2><p className="observation-order">순서: 자료 읽기 → 이 상자 체크 → {revealedFrameIndex < finalFrameIndex ? "다음 단계 열기" : "자료 추적 마치기"}</p><label className={`check-row${!observed ? " gi-pulse" : ""}`}><input type="checkbox" checked={observed} onChange={(event) => setObserved(event.target.checked)} /> {observationCheckLabel(revealedFrameIndex)}</label>{revealedFrameIndex < finalFrameIndex ? <button type="button" className={actionButtonClass(canRevealNextFrame(revealedFrameIndex, Number(observed), scenario.frames.length))} disabled={!canRevealNextFrame(revealedFrameIndex, Number(observed), scenario.frames.length)} onClick={() => { setRevealedFrameIndex(revealNextFrame(revealedFrameIndex, Number(observed), scenario.frames.length)); setObserved(false); setFeedback(""); }}>다음 시간 단계 열기</button> : <button type="button" className={actionButtonClass(observed)} disabled={!observed} onClick={() => changeStage("review")}>자료 추적 마치기</button>}</div><ThermalWorkbench scenario={scenario} frameIndex={revealedFrameIndex} /><p aria-live="polite" className="sr-only">새 시간 단계: {scenario.frames[revealedFrameIndex].timeLabel} 자료가 열렸어요.</p><TemperatureChart scenario={scenario} revealedFrameIndex={revealedFrameIndex} /><TemperatureTable scenario={scenario} revealedFrameIndex={revealedFrameIndex} /></>}
    {stage === "review" && <section className="decision-panel"><h2>마지막 자료를 보고 방향을 확인해요</h2>{finalAnswers.map((answer, index) => <fieldset className="choice-group" key={answer.id}><legend>{finalAnswers.length > 1 ? `정거장 ${"ABC"[index]}의 최종 방향` : "최종 방향"}</legend><p>{answer.textAlternative}</p>{directionOptions(answer).map((option) => <label key={option.id}><input type="radio" name={`final-${scenario.id}-${answer.id}`} checked={finalDirections[answer.id] === option.id} onChange={() => setChoice(setFinalDirections, finalDirections, answer.id, option.id)} /> {directionLabel(scenario, option.fromId, option.toId, option.id)}</label>)}</fieldset>)}<button type="button" className={actionButtonClass(choicesComplete(finalAnswers, finalDirections))} disabled={!choicesComplete(finalAnswers, finalDirections)} onClick={reviewDirections}>방향 확인하기</button>{feedback && <p className="feedback" role="status">{feedback}</p>}</section>}
    {stage === "mode" && <section className="decision-panel"><h2>열이 주로 어떻게 갔을까요?</h2><details className="mode-help"><summary>전도·대류·복사 뜻 다시 보기</summary><ul>{modeExplanations.map(([name, detail]) => <li key={name}><strong>{name}:</strong> {detail}</li>)}</ul></details>{finalAnswers.map((answer, index) => <fieldset className="choice-group" key={answer.id}><legend>{finalAnswers.length > 1 ? `정거장 ${"ABC"[index]}의 열이 가는 방법` : "열이 가는 방법 선택"}</legend>{modeChoices.map((mode) => <label key={mode}><input type="radio" name={`mode-${scenario.id}-${answer.id}`} checked={selectedModes[answer.id] === mode} onChange={() => setChoice(setSelectedModes, selectedModes, answer.id, mode)} /> {modeChoiceLabels[mode]}</label>)}</fieldset>)}<button type="button" className={actionButtonClass(allModesSelected)} disabled={!allModesSelected} onClick={reviewModes}>근거 고르기</button>{feedback && <p className="feedback" role="status">{feedback}</p>}</section>}
    {stage === "evidence" && <section className="evidence-panel"><h2>근거를 연결해요</h2><p>이 사건에 꼭 필요한 근거를 모두 골라요.</p><div className="evidence-grid">{scenario.evidence.map((item) => <label className={`evidence-card ${evidence.includes(item.id) ? "selected" : ""}`} key={item.id}><input type="checkbox" checked={evidence.includes(item.id)} onChange={(event) => setEvidence(event.target.checked ? [...evidence, item.id] : evidence.filter((id) => id !== item.id))} /><strong>{item.title}</strong><span>{item.detail}</span></label>)}</div><p className="hint" role="status">{selectedRequiredEvidenceCount}/{scenario.requiredEvidenceIds.length}개를 골랐어요. {hasDistractorEvidence ? "맞지 않는 카드는 다시 눌러 빼 주세요." : "필요한 근거만 골라야 기록을 볼 수 있어요."}</p><button type="button" className={actionButtonClass(allEvidenceSelected)} disabled={!allEvidenceSelected} onClick={() => changeStage("result")}>추적 기록 보기</button></section>}
    {stage === "result" && <section className="result-panel"><p className="completion-stamp">추적 완료</p><h2>열이 어떻게 움직였는지 정리</h2><dl><div><dt>내 처음 예측</dt><dd>{predictionAnswers.map((answer) => directionLabel(scenario, answer.fromId, answer.toId, prediction[answer.id])).join(" · ")}</dd></div><div><dt>예측 시점의 방향</dt><dd>{predictionAnswers.map((answer) => directionLabel(scenario, answer.fromId, answer.toId, answer.direction)).join(" · ")}</dd></div><div><dt>마지막 자료의 방향</dt><dd>{finalAnswers.map((answer) => directionLabel(scenario, answer.fromId, answer.toId, answer.direction)).join(" · ")}</dd></div><div><dt>예측 시점과 마지막 비교</dt><dd>{finalAnswers.map((answer, index) => { const initial = predictionAnswers[index]; return frameComparisonStatus(initial?.direction ?? "", answer.direction, initial?.id ?? "", answer.id); }).join(" · ")}</dd></div><div><dt>까닭</dt><dd>{scenario.evidence.filter((item) => scenario.requiredEvidenceIds.includes(item.id) && evidence.includes(item.id)).map((item) => item.title).join(" · ")}</dd></div><div><dt>열이 가는 방법</dt><dd>{finalAnswers.map((answer) => modeLabels[answer.mode]).join(" · ")}</dd></div><div><dt>이 결과가 맞는 조건</dt><dd>{scenario.limitationText}</dd></div></dl><p className="reality-note">실제 생활에서는 전도·대류·복사가 함께 나타날 수 있어요. 이 사건은 비교하기 쉽게 만든 연습 자료예요.</p><button type="button" className={actionButtonClass(true)} onClick={onComplete}>{number === 5 ? "전체 활동 마무리" : "다음 사건으로"}</button></section>}
