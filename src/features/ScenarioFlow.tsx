...
export function ScenarioFlow({ scenario, number, onComplete }: { scenario: ThermalScenario; number: number; onComplete: () => void }) {
  ...
  return <section className="scenario-flow" aria-labelledby="scenario-title">
    ...
    {stage === "evidence" && <section className="evidence-panel"><h2>근거를 연결해요</h2><p>이 사건에 꼭 필요한 근거를 모두 골라요.</p><div className="evidence-grid">{scenario.evidence.map((item) => <label className={`evidence-card ${evidence.includes(item.id) ? "selected" : ""}`} key={item.id}><input type="checkbox" checked={evidence.includes(item.id)} onChange={(event) => setEvidence(event.target.checked ? [...evidence, item.id] : evidence.filter((id) => id !== item.id))} /><strong>{item.title}</strong><span>{item.detail}</span></label>)}</div>{!allEvidenceSelected && <p className="hint">필요한 근거를 모두 골라야 기록을 볼 수 있어요.</p>}<button type="button" className={actionButtonClass(allEvidenceSelected)} disabled={!allEvidenceSelected} onClick={() => changeStage("result")}>추적 기록 보기</button></section>}
