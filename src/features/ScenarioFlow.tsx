...
export function ScenarioFlow({ scenario, number, onComplete }: { scenario: ThermalScenario; number: number; onComplete: () => void }) {
  ...
  return <section className="scenario-flow" aria-labelledby="scenario-title">
    ...
    {stage === "result" && <section className="result-panel"><p className="completion-stamp">추적 완료</p><h2>열 이동 추적 기록</h2><dl><div><dt>시작 온도 차</dt><dd>{scenario.bodies.slice(0, 2).map((body) => `${body.label} ${body.initialTemperatureC}°C`).join(" / ")}</dd></div><div><dt>시간에 따른 변화</dt><dd>{scenario.frames.map((frame) => `${frame.timeLabel} ${scenario.bodies.slice(0, 2).map((body) => `${frame.temperaturesC[body.id]}°C`).join("·")}`).join(" → ")}</dd></div><div><dt>열의 알짜 방향</dt><dd>{finalEdge?.textAlternative}</dd></div><div><dt>주된 방식</dt><dd>{selectedMode ? modeLabels[selectedMode] : ""}</dd></div><div><dt>모형의 한계</dt><dd>{scenario.limitationText}</dd></div></dl><p className="reality-note">현실에서는 전도·대류·복사가 함께 나타날 수 있어요. 이 사건은 한 방식을 비교하려고 단순화했어요.</p><button type="button" className="button primary" onClick={onComplete}>{number === 5 ? "전체 활동 마무리" : "다음 사건으로"}</button></section>}
