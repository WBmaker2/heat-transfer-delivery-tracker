...
function DialogContent({ dialog }: { dialog: Exclude<DialogName, null> }) {
  ...
  return <><p>활동 순서: 조건 확인 → 방향 예측 → 시점별 온도 읽기 → 방향 확인 → 방식과 근거 연결 → 기록 확인</p><ul>{modeExplanations.map(([name, detail]) => <li key={name}><strong>{name}:</strong> {detail}</li>)}</ul><p>화살표는 열이 든 물건이 아니라, 열이 전체적으로 이동하는 방향을 뜻해요.</p></>;
}
