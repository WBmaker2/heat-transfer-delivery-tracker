...
export function TrackerApp() {
  ...
  return <div className="app-shell"><AppHeader scenarioLabel={scenarioLabel} onOpenHelp={() => openDialog("help")} onOpenTeacher={() => openDialog("teacher")} onOpenUpdates={() => openDialog("updates")} />
    <main id="main-content" tabIndex={-1}>{!started ? <ModelGuide onComplete={() => setStarted(true)} /> : finished ? <section className="finish-panel"><p className="completion-stamp">전체 추적 완료</p><h1>다섯 사건을 모두 살펴봤어요.</h1><p>온도 숫자, 시간 변화, 열이 가는 방향, 이동 방식을 함께 읽었어요.</p><button type="button" className="button primary" onClick={() => { setStarted(false); setScenarioIndex(0); setFinished(false); }}>처음부터 다시 보기</button></section> : <ScenarioFlow key={scenario.id} scenario={scenario} number={scenarioIndex + 1} onComplete={completeScenario} />}</main>
