...
export function TemperatureTable({ scenario, revealedFrameIndex }: { scenario: ThermalScenario; revealedFrameIndex: number }) {
  ...
  return <section className="table-panel" aria-labelledby="table-title"><h2 id="table-title">숫자 온도표</h2><p>그래프와 같은 숫자 자료예요. 아직 열지 않은 시간 단계는 보이지 않아요.</p>
    <div className="table-scroll"><table><thead><tr><th scope="col">시간 단계</th>{bodies.map((body) => <th key={body.id} scope="col">{body.label}</th>)}<th scope="col">열이 가는 방향</th></tr></thead>
    ...
    <div className="temperature-table-cards" aria-label="작은 화면용 숫자 온도표">{visibleFrames.map((frame) => <article key={frame.timeStep} className={frame.timeStep === revealedFrameIndex ? "current-row" : ""}><h3>{frame.timeLabel}</h3>{bodies.map((body) => <p key={body.id}><strong>{body.label}</strong><span>{frame.temperaturesC[body.id]}°C</span></p>)}<p><strong>열이 가는 방향</strong><span>{frame.edges.map((edge) => edge.textAlternative).join(" / ")}</span></p></article>)}</div>
  </section>;
}
