...
export function ThermalWorkbench({ scenario, frameIndex }: { scenario: ThermalScenario; frameIndex: number }) {
  ...
  const displayedBodies = scenario.bodies.filter((body) => edgeEndpointIds.includes(body.id));
  ...
  return (
    <section className="workbench" aria-labelledby="workbench-title">
      ...
      <div className="body-row">
        {displayedBodies.map((body, index) => <div className={`thermal-body ${index === 0 ? "first" : "second"}`} key={body.id}><span>{body.label}</span><strong>{frame.temperaturesC[body.id]}°C</strong><small>{body.material}</small></div>)}
      </div>
