...
test("removes the starter preview and keeps the educational content wired", async () => {
  const [page, layout, packageJson, scenarios, styles, workbench, table, dialog, chart, modelGuide] = await Promise.all([
  ...
  assert.match(styles, /@media \(max-width:700px\)/); assert.match(styles, /temperature-table-cards/); assert.match(styles, /prefers-reduced-motion/); assert.match(styles, /\.gi-pulse/); assert.match(styles, /@keyframes gi-pulse/); assert.match(styles, /focus-visible/);
  ...
  assert.match(flow, /자료 추적 마치기/); assert.match(flow, /actionButtonClass\(observed\)/); assert.match(flow, /actionButtonClass\(allEvidenceSelected\)/);
