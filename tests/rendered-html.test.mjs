...
test("removes the starter preview and keeps the educational content wired", async () => {
  const [page, layout, packageJson, scenarios, styles, workbench, table, dialog, chart, modelGuide, trackerApp] = await Promise.all([
    ...
    readFile(new URL("../src/visualization/ThermalWorkbench.tsx", import.meta.url), "utf8"), readFile(new URL("../src/visualization/TemperatureTable.tsx", import.meta.url), "utf8"), readFile(new URL("../src/components/Dialog.tsx", import.meta.url), "utf8"), readFile(new URL("../src/visualization/TemperatureChart.tsx", import.meta.url), "utf8"), readFile(new URL("../src/features/ModelGuide.tsx", import.meta.url), "utf8"), readFile(new URL("../src/app/TrackerApp.tsx", import.meta.url), "utf8"),
  ]);
  ...
  assert.match(styles, /@media \(max-width:700px\)/); assert.match(styles, /temperature-table-cards/); assert.match(styles, /progress-hint/); assert.match(styles, /observation-order/); assert.match(styles, /prefers-reduced-motion/); assert.match(styles, /\.gi-pulse/); assert.match(styles, /@keyframes gi-pulse/); assert.match(styles, /focus-visible/);
  ...
  assert.match(flow, /자료 추적 마치기/); assert.match(flow, /actionButtonClass\(observed\)/); assert.match(flow, /actionButtonClass\(allEvidenceSelected\)/); assert.match(flow, /areExactlyRequiredEvidenceSelected/); assert.match(flow, /selectedRequiredEvidenceCount/); assert.match(flow, /progress-hint/); assert.match(flow, /observation-order/); assert.match(flow, /requiredEvidenceIds\.includes\(item\.id\)/);
  assert.match(trackerApp, /className="button primary gi-pulse"/);
  assert.match(modelGuide, /gi-pulse/); assert.match(modelGuide, /ready \? " gi-pulse"/);
